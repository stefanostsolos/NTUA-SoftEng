...
test('login test', done => {
    ...
    os.execCommand('se2118 login --username not_valid --passw test', function (retVal) {
        const last = retVal.slice(-4);
        callback(cases(last));
    });
});
...
test('login test', done => {
    ...
    os.execCommand('se2118 login --username admin --passw test', function (retVal) {
        const last = retVal.slice(-4);
        callback(cases(last));
    });
});
...
test('admin passesupd test', done => {
    ...
    os.execCommand(`se2118 admin --passesupd --source ${__dirname}/../sample_imports.csv`, function (retVal) {
        const last = retVal.slice(-4);
        callback(cases(last));
    });
});
